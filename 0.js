...
function convertDate(date = "") {
  if (!date) {
    return `${new Date().toDateString().slice(0, 4)}${addZero(
      new Date().getDate()
    )}/${addZero(
      new Date().getMonth() + 1
    )}/${new Date().getFullYear()} - 23:59`;
  } else {
    return `${new Date(date).toDateString().slice(0, 4)}${addZero(
      new Date(date).getDate(date)
    )}/${addZero(new Date(date).getMonth() + 1)}/${new Date(
      date
    ).getFullYear()} - ${new Date(date).toTimeString().slice(0, 5)}`;
  }
}
//-----------------------------------------------------
...
function template(id, status, dateTime, name) {
  return `
        ...
        <span class="date">${convertDate(new Date(dateTime)).slice(
          0,
          14
        )}</span>
...
function renderTemplate(data, element, insertPosition) {
  data.forEach((item) => {
    element.insertAdjacentHTML(
      insertPosition,
      template(item.id, item.status, item.dateTime, item.name)
    );
  });
}
//-----------------------------------------------------
      ...
      if (countDisplay >= display.length) {
        ...
        if (countShow === 0 && document.querySelector(".notThing") === null) {
          list.insertAdjacentHTML(
            "afterbegin",
            `<p class="notThing">You don't have any task here</p>`
          );
        }
      } else {
        display[countDisplay++].classList.add("show");
...
function showAllTask() {
  ...
  if (
    list.querySelectorAll(".task").length === 0 &&
    document.querySelector(".notThing") === null
  ) {
    list.insertAdjacentHTML(
      "afterbegin",
      `<p class="notThing">You don't have any task here</p>`
