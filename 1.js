...
tabName.forEach((item) => {
  item.addEventListener("click", function () {
    ...
    document.querySelector(".tabList .line").style.left =
      item.offsetLeft + "px";
    document.querySelector(".tabList .line").style.width =
      item.offsetWidth + "px";

    document.querySelectorAll(".tabContent ul").forEach((ul) => {
      if (ul.dataset.pane === item.id) {
        document
          .querySelector(".tabContent ul.active")
...
document.querySelectorAll(".tabPaneShow li").forEach((item) => {
  item.addEventListener("click", function () {
    document
      .querySelector(".tabPaneShow li.active")
      ?.classList.remove("active");
    item.classList.add("active");
  });
});
