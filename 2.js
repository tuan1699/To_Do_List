...
function nameClick(_this) {
  ...
  const tabShowActive = document.querySelector(
    ".tabPaneShow.active li.active"
  )?.id;

  console.log(tabShowActive);
...
buttonAdd.addEventListener("click", function () {
  ...
  list.insertAdjacentHTML(
    "afterbegin",
    template(+idMax + 1, "pending", dateTime, name)
  );
  setTimeout(() => {
...
inputTask.addEventListener("keyup", function (e) {
  if (e.key !== "Backspace" && inputTask.value) {
    inputTask.value =
      inputTask.value[0].toUpperCase() + inputTask.value.slice(1);
  }
  if (e.key === "Enter") {
    buttonAdd.click();
  }
...
function deleteClick(_this) {
  ...
  const test = data.find((item, index) => {
...
function editClick(_this) {
  ...
  console.log(pDateTime.innerText);
...
buttonSave.addEventListener("click", function () {
  ...
  const taskData = data.find((item) => {
    return +item.id === +taskEdit.dataset.id;
  });
  taskData.name = name;
...
setInterval(() => {
  for (const item of data) {
    if (
      new Date().getTime() >= new Date(item.dateTime).getTime() &&
      item.status === "pending"
    ) {
